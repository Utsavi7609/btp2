...
def per_clip_analysis(df_llama, df_mistral):
    """
    Average predictions per clip across participants.
    Follows check_reported_vs_inferred.py approach.
    """
    results_list = []
    for df, mname in [(df_llama, 'Llama'), (df_mistral, 'Mistral')]:
        df = df.copy()
        ...
        clip_avg = df.groupby('clip_id').agg({
            ...
            'self_reported_valence': f'true_v',
            'self_reported_arousal': f'true_a',
    ...
    if results_list:
        combined = results_list[0].join(results_list[1][[c for c in results_list[1].columns
                                                          if 'Mistral' in c]], how='outer')
...
print("="*65)
print("  MODEL COMPARISON: Llama vs Mistral")
...
for model_name, results_file in model_configs:
    if not os.path.exists(results_file):
        print(f"\n⚠️  {results_file} not found. Run the inference script first.")
        continue
...
# Per-clip analysis if both files exist
if len(loaded_dfs) == 2:
    per_clip_analysis(
        loaded_dfs["Llama-3.3-70B"],
        loaded_dfs["Mixtral-8x7B"]
    )
...
import os  # already imported above but guard for standalone run
