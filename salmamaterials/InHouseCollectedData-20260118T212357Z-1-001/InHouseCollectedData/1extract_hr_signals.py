...
def extract_bpm_window(file_path, end_dt, window_sec=WINDOW_SEC):
    ...
    for entry in data:
        try:
            ...
            if bpm and start_dt <= entry_time <= end_dt:
                readings.append((entry_time, float(bpm)))

        except Exception:
            continue

    # SORT HR READINGS CHRONOLOGICALLY
    readings.sort(key=lambda x: x[0])

    # KEEP ONLY BPM VALUES
    bpm_readings = [r[1] for r in readings]
...
# REMOVE ANY PARTICIPANTS NOT IN OUR MAP (e.g., SALMA)
df_master = df_master[df_master['participant'].isin(PARTICIPANT_MAP.keys())]
