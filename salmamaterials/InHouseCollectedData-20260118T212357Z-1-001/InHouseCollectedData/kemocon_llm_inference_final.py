...
def load_e4_hr(hr_file):
    """
    Load E4 HR CSV.
    Safely handles both header and headerless formats.
    """
    try:
        df = pd.read_csv(hr_file)
        if 'value' in df.columns:
            hr_values = pd.to_numeric(df['value'], errors='coerce').dropna().values
            return pd.DataFrame({'hr': hr_values})
    except Exception:
        pass
    
    # Fallback to headerless logic
    df = pd.read_csv(hr_file, header=None)
    hr_values = pd.to_numeric(df.iloc[2:, 0], errors='coerce').dropna().values
    return pd.DataFrame({'hr': hr_values})
...
def load_e4_ibi(ibi_file):
    ...
    try:
        ...
        if 'value' in df.columns:
            ibi_sec = pd.to_numeric(df['value'], errors='coerce').dropna().values
            return pd.DataFrame({'ibi_sec': ibi_sec})
    except Exception:
        pass
    ...
    ibi_sec = pd.to_numeric(df.iloc[1:, 1], errors='coerce').dropna().values
    return pd.DataFrame({'ibi_sec': ibi_sec})
...
def normalize_participant_id(raw_id):
    """Normalize participant IDs like 'P01.external', '1', 'p1' → integer."""
    try:
...
def format_sensor_readings(r):
    hr_seq = r['hr_sequence']
    hr_seq_str = hr_seq if isinstance(hr_seq, str) else str(hr_seq)
    
    return (
        f"[Heart Rate Sequence (BPM)]: {hr_seq_str}. "
        f"[Statistical Summary]: "
        f"Mean = {r['hr_mean']} BPM, "
        f"Std = {r['hr_std']} BPM, "
        f"Min = {r['hr_min']} BPM, "
        f"Max = {r['hr_max']} BPM, "
        f"Drift (last - first) = {r['hr_drift']} BPM, "
        f"HRV RMSSD = {r['hrv_rmssd']} ms."
    )


def create_kemocon_prompt(row, df):
    """
    Health-LLM 'All' context + Few-Shot Examples
    """
    # ── Health Context (hc) ──────────────────────────────────────────────────
    health_ctx = (
    ...
    # ── Few-Shot Examples ────────────────────────────────────────────────────
    valid_df = df.dropna(subset=['true_valence', 'true_arousal'])
    candidates = valid_df[valid_df.index != row.name]
    num_shots = min(3, len(candidates))
    
    few_shot_examples = ""
    if num_shots > 0:
        # Use random_state=42 for reproducibility across the run
        sampled_examples = candidates.sample(n=num_shots, random_state=42)
        few_shot_examples += "### Here are a few reference examples mapping physiological data to true scores:\n\n"
        for _, ex_row in sampled_examples.iterrows():
            ex_sensors = format_sensor_readings(ex_row)
            few_shot_examples += (
                f"**Example Input:**\n"
                f"Sensor Readings: {ex_sensors}\n"
                f"**Example Expected Output JSON:**\n"
                f"{{\"valence\": {ex_row['true_valence']}, \"arousal\": {ex_row['true_arousal']}}}\n\n"
            )

    # ── Temporal Context for Target Participant ──────────────────────────────
    temporal_ctx = format_sensor_readings(row)
    ...
    prompt = (
        ...
        f"### CURRENT TARGET PARTICIPANT INFERENCE:\n"
        f"### Sensor Readings: {temporal_ctx}\n\n"

        f"### Question: Based on the physiological data above and using the provided examples to calibrate "
        f"your predictions, predict the current participant's Valence and Arousal scores on a 1–5 integer scale.\n\n"
...
features_df['llm_question'] = features_df.apply(lambda r: create_kemocon_prompt(r, features_df), axis=1)
features_df.to_csv('kemocon_features.csv', index=False)
            ...
            try:
                ...
                if "429" in err or "rate" in err.lower():
                    ...
                    if "model_decommissioned" in err:
                        retries = 5  # Give up immediately
                        raw = f"ERROR: decommissioned"
                    else:
                        time.sleep(5)
                        
                if retries >= 5:
                    if not raw: raw = f"ERROR: {err}"
                    print(f"    [Skip]")
                    # Avoid appending strings to numeric lists
                    inf_v, inf_a = np.nan, np.nan
        
        # Make sure row size matches dataframe columns
...
mistral_results = run_inference_loop(valid_df, "llama-3.1-8b-instant",    "kemocon_mistral_results.csv") # Used 8B since mixtral decommissioned
