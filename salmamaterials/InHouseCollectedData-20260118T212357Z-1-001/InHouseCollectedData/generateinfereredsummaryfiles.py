...
# 1. Load the LLM results
if not os.path.exists('llm_results_qwen_v3_shifted.csv'):
    print("Error: llm_results_qwen_v3_shifted.csv not found!")
else:
    df = pd.read_csv('llm_results_qwen_v3_shifted.csv')
    df['clip_id'] = clean_id(df['clip_title'])
    ...
    v_summary['avg_inferred_valence'] = v_summary.mean(axis=1)
    v_summary.to_csv('inferred_valence_summary_qwen_FS.csv')
    ...
    a_summary['avg_inferred_arousal'] = a_summary.mean(axis=1)
    a_summary.to_csv('inferred_arousal_summary_qwen_FS.csv')
